...
| `onionmail gui`    | **Native masaüstü penceresi** (PySide6) — siyah-beyaz "Messager" (*Who Am I* teması), ters barlar midnight. Menü: `Email · Settings · Help`. Login/Kaydol ekranı, ayrı "New Message" penceresi, Collector (sandbox). `--local` ile dm1'de doğrudan Maildir. Kurulum: `pip install -e ".[gui]"`. |
| `onionmail tui`    | Aynı arayüzün **terminal** sürümü (Textual, yerel mod). SSH üzerinden dm1'de kullanışlı. |
...
* **Tek kutu** (`accounts.enabled=false`): GUI/TUI Maildir'i doğrudan okur; login yok. dm1'de veya SSH ile.
...
Gerçek kurulum (dm1 / Debian 13): **`docs/tor-setup.md`**.
...
- **Gerçek onion round-trip henüz test edilmedi** (dm1 kurulunca).
